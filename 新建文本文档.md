...
| 指标名称 | 英文 | 公式 | 物理含义与特性 |
| :--- | :--- | :--- | :--- |
| **准确率** | Accuracy | $$ \frac{TP + TN}{TP + TN + FP + FN} $$ | 模型正确分类样本占总样本的比例。**局限**：在类别极度不平衡时失效（Accuracy Paradox）。 |
| **精确率** | Precision | $$ \frac{TP}{TP + FP} $$ | 查准率。预测为正例的样本中，实际为正例的比例。**适用**：对误报（False Positive）敏感的场景（如垃圾邮件过滤）。 |
| **召回率** | Recall | $$ \frac{TP}{TP + FN} $$ | 查全率（亦称灵敏度 Sensitivity）。实际为正例的样本中，被正确预测的比例。**适用**：对漏报（False Negative）敏感的场景（如疾病筛查）。 |
| **F1分数** | F1-Score | $$ 2 \cdot \frac{\text{Precision} \cdot \text{Recall}}{\text{Precision} + \text{Recall}} $$ | 精确率与召回率的**调和平均数**。**特性**：综合衡量模型性能，适用于类别不平衡场景。 |

### 2. 阈值与曲线指标

* **ROC曲线 (Receiver Operating Characteristic Curve)**
  * **定义**：以假阳性率 $FPR = \frac{FP}{FP + TN}$ 为横轴，真阳性率 $TPR = \frac{TP}{TP + FN}$ (Recall) 为纵轴绘制的曲线。
...
| **平均绝对百分比误差 (MAPE)** | $$ \frac{100\%}{n}\sum \left| \frac{y_i - \hat{y}_i}{y_i} \right| $$ | 衡量相对误差。当真实值 $y_i$ 接近 0 时数值不稳定。 |
...
正确分类的像素点占总像素点的比例。
$$ PA = \frac{\sum_{i=0}^{k} TP_i}{\sum_{i=0}^{k}(TP_i + FP_i)} $$
...
### 2. 图像复原 (超分/去噪)
* **PSNR (Peak Signal-to-Noise Ratio)**：峰值信噪比。基于像素误差（MSE）计算。$$ \text{PSNR} = 10 \cdot \log_{10}\left(\frac{MAX_I^2}{\text{MSE}}\right) $$。数值越高表示失真越小，但与人眼主观感知并不完全一致。
