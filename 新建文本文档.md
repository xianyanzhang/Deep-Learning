明白，为了适用于学术文档整理和专业学习，以下是重新编排的**深度学习模型评价指标综述**。

该版本采用学术规范的语言风格，统一了符号定义，涵盖了分类、回归、计算机视觉（检测/分割/生成）及自然语言处理等核心领域的评价体系。
...
| 指标名称 | 英文 | 公式 | 物理含义与特性 |
| :--- | :--- | :--- | :--- |
| **准确率** | Accuracy | $\frac{TP + TN}{TP + TN + FP + FN}$ | 模型正确分类样本占总样本的比例。<br>**局限**：在类别极度不平衡时失效（Accuracy Paradox）。 |
| **精确率** | Precision | $\frac{TP}{TP + FP}$ | 查准率。预测为正例的样本中，实际为正例的比例。<br>**适用**：对误报（False Positive）敏感的场景（如垃圾邮件过滤）。 |
| **召回率** | Recall | $\frac{TP}{TP + FN}$ | 查全率（亦称灵敏度 Sensitivity）。实际为正例的样本中，被正确预测的比例。<br>**适用**：对漏报（False Negative）敏感的场景（如疾病筛查）。 |
| **F1分数** | F1-Score | $2 \cdot \frac{\text{Precision} \cdot \text{Recall}}{\text{Precision} + \text{Recall}}$ | 精确率与召回率的**调和平均数**。<br>**特性**：综合衡量模型性能，适用于类别不平衡场景。 |

### 2. 阈值与曲线指标
*   **ROC曲线 (Receiver Operating Characteristic Curve)**
    *   **定义**：以假阳性率 (FPR) 为横轴，真阳性率 (TPR/Recall) 为纵轴绘制的曲线。
...
| 指标名称 | 公式 | 特性与适用性 |
| :--- | :--- | :--- |
| **平均绝对误差 (MAE)** | $\frac{1}{n}\sum_{i=1}^{n}|y_i - \hat{y}_i|$ | L1 范数损失。对离群值（Outliers）具有鲁棒性，解释性强。 |
| **均方误差 (MSE)** | $\frac{1}{n}\sum_{i=1}^{n}(y_i - \hat{y}_i)^2$ | L2 范数损失。对大误差惩罚更重，对离群值敏感。 |
| **均方根误差 (RMSE)** | $\sqrt{\text{MSE}}$ | 量纲与原变量一致，便于直观解释。 |
| **决定系数 ($R^2$)** | $1 - \frac{\sum(y_i - \hat{y}_i)^2}{\sum(y_i - \bar{y})^2}$ | 衡量模型拟合优度，表示模型解释了数据方差的比例。最佳值为 1。 |
| **平均绝对百分比误差 (MAPE)** | $\frac{100\%}{n}\sum |\frac{y_i - \hat{y}_i}{y_i}|$ | 衡量相对误差。当真实值 $y_i$ 接近 0 时数值不稳定。 |
...
*   通常设定 $\text{IoU} \geq 0.5$ 为检测正确 (TP)。

### 2. 平均精度 (AP) 与 平均精度均值 (mAP)
...
$$ \text{mIoU} = \frac{1}{k+1} \sum_{i=0}^{k} \frac{TP_i}{TP_i + FP_i + FN_i} $$
...
### 2. 图像复原 (超分/去噪)
*   **PSNR (Peak Signal-to-Noise Ratio)**：峰值信噪比。基于像素误差（MSE）计算。数值越高表示失真越小，但与人眼主观感知并不完全一致。
...
---
*注：本文档旨在提供深度学习评价指标的学术性综述，相关公式与定义参照机器学习标准教材及相关领域顶会（CVPR, ACL, NeurIPS）通用标准。*